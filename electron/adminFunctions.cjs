...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error(
    "Supabase URL or Anon Key is missing. Make sure you have a .env file with SUPABASE_URL and SUPABASE_ANON_KEY."
  );
}
...
async function loginAdmin(email, password) {
  ...
  // Check if stored password is hashed (starts with $2 for bcrypt)
  let isValid = false;
  const isHashed =
  ...
  if (isHashed) {
    ...
    isValid = password === adminUser.password;
  }
