...
export default function Dashboard({
  ...
  const fetchStudents = async () => {
    try {
      ...
      const stemCount =
        enrolledData?.filter((s) => s.strand === "STEM").length || 0;
      const abmCount =
        enrolledData?.filter((s) => s.strand === "ABM").length || 0;
      const tvlIctCount =
        enrolledData?.filter((s) => s.strand === "TVL-ICT").length || 0;
      const humssCount =
        enrolledData?.filter((s) => s.strand === "HUMSS").length || 0;
    ...
    <div className="relative flex min-h-screen overflow-hidden">
      ...
      <div className="flex-1 ml-68 overflow-y-auto">
            ...
            <div className="bg-gradient-to-tr from-purple-400 to-purple-600 p-4 rounded-full shadow-lg">
              ...
              >
                ...
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222 4 2.222V20M1 14v5h22v-5"
                />
              </svg>
            </div>
          </div>
        ...
        <div className="mt-8 bg-white p-6 rounded-xl shadow-md flex-grow hover:scale-[1.02] transition-transform duration-300 cursor-pointer">
          <div className="flex justify-between items-center border-b-2 border-gray-300 pb-2 mb-4">
            <h2 className="text-lg font-bold text-gray-600">
              RECENT ALS ENROLLMENT APPLICATIONS:
            </h2>
            <button
              onClick={() => onNavigate("enrollment")}
              className="text-sm font-medium text-gray-600 hover:text-black bg-gray-100 hover:bg-gray-200 px-3 py-1 rounded-lg transition-all duration-200"
