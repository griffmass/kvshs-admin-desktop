...
import { Search, Eye, Trash2, Pencil, XCircle } from 'lucide-react';
...
export default function RegularStudent() {
  ...
  const fetchRegularStudents = async () => {
    try {
      const { data, error } = await supabase
        .from('NewStudents')
        .select('lrn, schoolYear, gradeLevel, psa, lname, fname, mname, bday, age, sex, birthplace, religion, motherTongue, indigenousPeople, fourPS, houseNumber, streetName, barangay, municipality, province, country, zipCode, pHN, pSN, pbrgy, pMunicipal, pProvince, pCountry, pZipCode, fatherFN, fatherMN, fatherLN, fatherCN, motherFN, motherMN, motherLN, motherCN, guardianFN, guardianLN, guardianCN, SNEP, pwdID, rlGradeLevelComplete, rlLastSYComplete, rlLastSchoolAtt, rlSchoolID, semester, track, strand, distanceLearning, enrollment_status, approved_at')
        .neq('strand', 'ALS')
  ...
  const filteredStudents = students.filter((student) => {
    ...
    return matchesSearch && matchesStrand && matchesGradeLevel && matchesAlphabetical && matchesLRN && matchesSex;
  }).sort((a, b) => {
  ...
  const handleUnenroll = (studentLrn: string | undefined, studentName?: string) => {
    if (!studentLrn) return;
    setConfirmAction({
      type: 'unenroll',
      studentLrn,
  ...
  return (
    <div className="w-full">
      <div className="border-b-2 border-gray-300 pb-2 mb-4">
        <h2 className="text-lg font-bold text-gray-800">LIST OF ALL REGULAR STUDENTS:</h2>
      </div>
                ...
                <tr key={student.lrn} className="bg-white border-b">
                  <td className="py-3 px-6">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => handleUnenroll(student.lrn, `${student.lname}, ${student.fname}`)}
                        className="flex items-center space-x-1 px-3 py-1 bg-orange-400 text-white text-xs rounded hover:bg-orange-600"
                        title="Unenroll"
                      >
                        <XCircle size={14} />
                        <span>Unenroll</span>
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6 border-b pb-4">
                <h2 className="text-2xl font-bold text-gray-800">
                  Confirm {confirmAction.type === 'delete' ? 'Delete' : 'Unenroll'}
                </h2>
                <button
                  onClick={() => {
                    setShowConfirmModal(false);
              ...
              <div className="mb-6">
                <p className="text-gray-600 text-lg">
                  Are you sure you want to {confirmAction.type === 'delete' ? 'delete' : 'unenroll'} the student "{confirmAction.studentName}"?
                </p>
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button
                  onClick={confirmAction.type === 'delete' ? handleConfirmDelete : handleConfirmUnenroll}
